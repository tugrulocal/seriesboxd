...
import { AlertTriangle, Maximize2, Minimize2, Check, Eye, Heart, Bookmark, MessageSquare, Star, X, Plus, ChevronLeft, ChevronRight, Captions } from 'lucide-react';
...
function WatchPage() {
    ...
    const currentSezonIndex = sezonlar.findIndex(s => s.season_id === seciliSezonId);
    const currentEpIndex = displayBolumler.findIndex(b => b.episode_number === parseInt(episode));
    let prevNavTarget = null; // { bolum, seasonId }
    let nextNavTarget = null;
    if (currentEpIndex > 0) {
        prevNavTarget = { bolum: displayBolumler[currentEpIndex - 1], seasonId: seciliSezonId };
    } else if (currentSezonIndex > 0) {
        const prevSezon = sezonlar[currentSezonIndex - 1];
    ...
    if (currentEpIndex < displayBolumler.length - 1 && currentEpIndex >= 0) {
        nextNavTarget = { bolum: displayBolumler[currentEpIndex + 1], seasonId: seciliSezonId };
    } else if (currentSezonIndex < sezonlar.length - 1) {
        const nextSezon = sezonlar[currentSezonIndex + 1];
                    ...
                    <div className="cinematic-player-wrapper">
                        ...
                        {magnetAramaDurumu === 'found' && (
                            <div className="player-source-bar">
                                <span className="player-ep-badge-bar">S{season}·E{episode}{currentEpisodeData?.name ? ` ${currentEpisodeData.name}` : ''}</span>
                        ...
                        <div className="player-main-area watch-player-area">
                            ...
                            >
                                ...
                                <div className="native-fs-blocker" onClick={e => { e.stopPropagation(); toggleFullscreen(); }} />
                                {activeSub?.url && (
                                    <SubtitleOverlay
                                        key={activeSub.url}
                                        ...
                                        <button className="wip-review-gonder-btn" disabled={!reviewText.trim() || reviewGonderiliyor}
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    setReviewText(''); setSpoilerVar(false); setReviewPanelAcik(false);
                ...
                <div className="wip-ep-dropdown" style={dropdownStyle} ref={dropdownRef}>
                    ...
                    <div className="wip-ep-dropdown-list">
                        {displayBolumler.map(bolum => {
                            const isCurrent = parseInt(episode) === bolum.episode_number;
